...
** Last update Sun May 17 21:46:09 2015 Barthelemy Gouby
*/
...
int		add_cell_to_list(t_cell **list, t_cell *cell_before, int x, int y)
{
  ...
  if (tmp == NULL)
    {
      *list = new_cell;
      (*list)->list_before = NULL;
      return (0);
    }
  while (tmp->next != NULL)
    tmp = tmp->next;
  tmp->next = new_cell;
  new_cell->list_before = tmp;
  return (0);
}
...
t_cell		*choose_and_pop_from_list(t_cell **list, t_dimensions *dimensions)
{
  t_cell	*tmp;
  t_cell	*best_cell;
  double	higher;
  ...
  if (best_cell != NULL && best_cell->list_before != NULL)
    {
      best_cell->list_before->next = best_cell->next;
      if (best_cell == *list)
	*list = best_cell->list_before;
    }
  if (best_cell != NULL && best_cell->next != NULL)
    {
      best_cell->next->list_before = best_cell->list_before;
      if (best_cell == *list)
	*list = best_cell->next;
    }
  if (best_cell != NULL && best_cell->next == NULL && best_cell->list_before == NULL)
    *list = NULL;
  return (best_cell);
}
  ...
  while (graph->x != (dimensions->width - 1) || graph->y != (dimensions->height - 1))
    {
      ...
      if ((graph = choose_and_pop_from_list(&to_visit, dimensions)) == NULL)
	{
